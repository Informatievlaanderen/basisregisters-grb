namespace Grb.Building.Tests.JobProcessor
{
    ...
    public class GivenPreparingJob
    {
        [Fact]
        public async Task ThenNothing()
        {
            ...
            var job = new Job(DateTimeOffset.Now.AddMinutes(-10), JobStatus.Preparing);
            ...
            //assert
            jobRecordsProcessor.Verify(x => x.Process(job.Id, It.IsAny<Func<JobGrbRecord, bool>>(), It.IsAny<bool>(), It.IsAny<CancellationToken>()), Times.Never);
